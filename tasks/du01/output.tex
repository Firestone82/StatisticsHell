...
pro \tableValue{\rangeValues}{1} testovacích systémů. Zjištěný nárůst FPS se pohyboval v rozmezí \tableValue{\minValues}{1} FPS
až \tableValue{\maxValues}{1} FPS. Nárůst FPS v testu č. 24 byl na základě metody vnitřních hradeb identifikován jako odlehlé pozorování
a nebude zahrnut do dalšího zpracování. Možné příčiny vzniku odlehlých pozorování jsou: Chyba při testování vzniklá náhlým výkyvem výkonu počítače
nebo chyba hardwaru výpočetní stanice. Dále uvedené výsledky tedy pocházejí z analýzy nárůstů FPS zjištěných u \tableValue{\rangeValues}{3}
testovacích systémů. Průměrný nárůst FPS byl \tableValue{\meanValues}{3} FPS, směrodatná odchylka pak \tableValue{\sdValues}{3} FPS\@.
U poloviny testovacích cyklů nárůst FPS nepřekročil \tableValue{\medianValues}{3} FPS. V polovině případů se nárůst FPS pohyboval v
rozmezí \tableValue{\QfValues}{3} FPS až \tableValue{\QtValues}{3} FPS. Vzhledem k povaze měřené veličiny není variační koeficient vhodnou mírou
pro posouzení variability souboru.
...
Na základě grafického zobrazení (viz \figref{fig:qq}) u grafické karty Nvidia RTX 3070 Ti a výběrové šikmosti a špičatosti (viz \tabref{tab:characteristics-summary},
...
Na základě grafického zobrazení (viz \figref{fig:qq}) u grafické karty AMD Radeon RX 7700 XT a výběrové šikmosti a špičatosti (viz \tabref{tab:characteristics-summary},
výběrová šikmost a špičatost leží v intervalu (-2, 2)) lze předpokládat, že pozorovaný nárůst FPS má normální rozdělení. Dle pravidla 3$\sigma$ lze tedy očekávat,
